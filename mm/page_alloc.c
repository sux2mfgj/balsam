...
static inline uint32_t wait_table_bits(uint32_t size) { return ffz(~size); }

void memmap_init_zone(uint32_t size, int nid, uint32_t zone,
                      uint32_t start_pfn) {
    struct page *page;
    uint32_t end_pfn = start_pfn + size;
    uint32_t pfn;
...
#define memmap_init(size, nid, start_pfn) \
    memmap_init_zone((size), (nid), (zone), (start_pfn))

static void free_area_init_core(struct pglist_data *pgdat, uint32_t *zones_size,
                                uint32_t *zholes_size) {
    uint32_t i, j;
    ...
    for (j = 0; j < MAX_NR_ZONES; j++) {
        ...
        zone->wait_table = (wait_queue_head_t *)alloc_bootmem_node(
